...
export default function Auth() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t('Ошибка соединения. Попробуйте ещё раз.'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <div className="container mx-auto px-4 pt-28 pb-20 max-w-md">
        <h1 className="text-4xl font-bold tracking-tighter mb-8 dark:text-white">
          {mode === 'register' ? t('Регистрация') : mode === 'phone' ? t('Вход по телефону') : t('Вход')}
        </h1>
        ...
        {mode !== 'phone' && (
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-sm text-neutral-400">{t('или')}</span>
            <div className="flex-1 h-px bg-neutral-200 dark:bg-neutral-700" />
          </div>
        )}
        ...
        {mode !== 'phone' && (
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-neutral-500 dark:text-neutral-400 mb-1">{t('Пароль')}</label>
              <input
                type="password"
                value={password}
            ...
            <button type="submit" disabled={loading} className={btnClass}>
              {loading ? t('Загрузка...') : mode === 'login' ? t('Войти') : t('Зарегистрироваться')}
            </button>
          </form>
        )}
        ...
        {mode === 'phone' && (
          <form onSubmit={smsSent ? verifySms : sendSms} className="space-y-4">
            <div>
              <label className="block text-sm text-neutral-500 dark:text-neutral-400 mb-1">{t('Номер телефона')}</label>
              <input
                type="tel"
                value={phone}
            ...
            {smsSent && (
              <div>
                <label className="block text-sm text-neutral-500 dark:text-neutral-400 mb-1">{t('Код из SMS')}</label>
                <input
                  type="text"
                  value={smsCode}
                  ...
                  autoFocus
                />
                <p className="text-xs text-neutral-400 mt-1">{t('Код отправлен на')} {phone}</p>
              </div>
            )}
            {error && <p className="text-brand text-sm">{error}</p>}
            <button type="submit" disabled={loading} className={btnClass}>
              {loading ? t('Загрузка...') : smsSent ? t('Подтвердить') : t('Получить код')}
            </button>
            {smsSent && (
        ...
        <div className="mt-6 space-y-2 text-center text-sm text-neutral-500 dark:text-neutral-400">
          {mode !== 'phone' && (
            <p>
              {mode === 'login' ? t('Нет аккаунта?') : t('Уже есть аккаунт?')}{' '}
              <button
