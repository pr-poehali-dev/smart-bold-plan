      ...
      <div className="container mx-auto px-4 md:px-8 pt-28 pb-20 max-w-3xl">
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button onClick={() => navigate('/modeling')} className="px-8 py-3 bg-black dark:bg-white dark:text-black text-white text-sm uppercase tracking-widest rounded-xl hover:bg-red-600 transition-colors">
              Выбрать услугу
            </button>
          </div>
        ) : (
          <>
            <div className="space-y-4 mb-8">
              {items.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-5 border border-neutral-200 dark:border-neutral-800 rounded-2xl bg-neutral-50 dark:bg-neutral-900">
                  ...
                  <button onClick={() => remove(item.service_id)} className="text-neutral-400 hover:text-red-500 transition-colors">
                    <Icon name="X" size={16} />
                  </button>
                </div>
              ))}
            ...
            <div className="border-t border-neutral-200 dark:border-neutral-800 pt-6 flex items-center justify-between">
              ...
              <button
                onClick={checkout}
                disabled={ordering}
                className="px-10 py-4 bg-black dark:bg-white dark:text-black text-white text-sm uppercase tracking-widest rounded-xl hover:bg-red-600 dark:hover:bg-red-600 dark:hover:text-white transition-colors disabled:opacity-50"
              >
